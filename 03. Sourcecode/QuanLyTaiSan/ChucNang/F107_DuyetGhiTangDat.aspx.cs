...
public partial class ChucNang_F107_DuyetGhiTangDat : System.Web.UI.Page
{
    ...
    private void clear_form_data()
    {
        ...
        m_cbo_don_vi_su_dung_up.Enabled = true;
        m_cbo_dia_chi.Enabled = true;
        m_cbo_ly_do_thay_doi.Enabled = true;
    }

    private bool check_validate_data_is_ok()
    {
        if (m_cbo_dia_chi.Items.Count == 0 && m_e_form_mode == DataEntryFormMode.InsertDataState)
        {
    ...
    private void us_2_form_for_update()
    {
        ...
        m_cbo_don_vi_su_dung_up.Enabled = false;
        m_cbo_dia_chi.Enabled = false;
        m_cbo_ly_do_thay_doi.Enabled = false;
        US_DM_DAT v_us_dm_dat = new US_DM_DAT(CIPConvert.ToDecimal(m_us_gd_tang_giam_tai_san.dcID_TAI_SAN));
        m_lbl_ma_tai_san.Text = v_us_dm_dat.strMA_TAI_SAN;
        m_lbl_dt_khuon_vien.Text = v_us_dm_dat.dcDT_KHUON_VIEN.ToString("#,##0");
        m_lbl_gt_theo_so_ke_toan.Text = v_us_dm_dat.dcGT_THEO_SO_KE_TOAN.ToString("#,##0");
        m_lbl_so_nam_su_dung.Text = v_us_dm_dat.dcSO_NAM_DA_SU_DUNG.ToString("#,##0");
    ...
    protected void m_grv_danh_sach_dat_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            if (!e.CommandName.Equals(String.Empty) && !e.CommandName.Equals("Page"))
            {
                ...
                decimal v_dc_id_tg = CIPConvert.ToDecimal(m_grv_danh_sach_dat.DataKeys[rowIndex].Value);
                m_lbl_message.Text = "";
